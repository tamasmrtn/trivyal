...
# group_add in docker-compose adds the GID to the init process but gosu replaces
...
if [ -S "$DOCKER_SOCK" ]; then
    ...
    if ! getent group "$SOCK_GID" > /dev/null 2>&1; then
        addgroup --system --gid "$SOCK_GID" docker-host
    fi
    GROUP_NAME=$(getent group "$SOCK_GID" | cut -d: -f1)
    adduser trivyal "$GROUP_NAME" 2>/dev/null || true
fi
